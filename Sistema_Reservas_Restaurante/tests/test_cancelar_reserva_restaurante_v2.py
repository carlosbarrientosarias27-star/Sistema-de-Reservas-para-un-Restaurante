...
def restaurante_con_datos():
    """
    Configuración inicial: crea un restaurante con 2 mesas y 
    una reserva activa para usar en los tests de cancelación.
    """
    r = Restaurante()
    ...
    # Creamos una reserva inicial que intentaremos cancelar después
...
def test_cancelar_reserva_existente(restaurante_con_datos):
    ...
    1. Se elimine de la lista 'self.reservas'.
    2. El ID de la mesa se elimine de 'self.ocupacion_por_fecha'.
    """
...
def test_cancelar_reserva_inexistente(restaurante_con_datos):
    ...
    # La cancelamos una vez para que ya no exista en el sistema
    r.cancelar_reserva(reserva)
...
def test_cancelar_reserva_libera_solo_sus_mesas(restaurante_con_datos):
    ...
    Cancelar una NO debe liberar la mesa de la otra.
    """
    ...
    # La mesa 1 debe estar libre, pero la mesa 2 debe seguir bloqueada
    assert 1 not in r.ocupacion_por_fecha[fecha]
...
def test_cancelar_reserva_fecha_inexistente_en_mapa(restaurante_con_datos):
    """
    Caso Límite: Verifica que el código sea defensivo si por alguna razón
    la fecha de la reserva no existe en el diccionario de ocupación.
    """
    r, reserva, fecha = restaurante_con_datos
    
    # Forzamos la eliminación de la fecha en el mapa de ocupación (simulando error de estado)
    del r.ocupacion_por_fecha[fecha]
    
    # No debería lanzar KeyError gracias a la validación 'if reserva.fecha_hora in ...'
    r.cancelar_reserva(reserva)
    assert reserva not in r.reservas