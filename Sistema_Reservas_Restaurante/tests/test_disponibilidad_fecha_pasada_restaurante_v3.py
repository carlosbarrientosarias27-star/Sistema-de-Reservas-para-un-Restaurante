...
# --- TESTS PARA buscar_disponibilidad() ---
...
def test_disponibilidad_fecha_pasada(restaurante_con_mesas):
    ...
    Observación: La lógica actual permite consultar el pasado pues 
    simplemente verifica que no haya registros en 'ocupacion_por_fecha'.
    """
    fecha_ayer = datetime.now() - timedelta(days=1)
    disponibles = restaurante_con_mesas.buscar_disponibilidad(fecha_ayer, 1)
    
    # El sistema devuelve las mesas ya que técnicamente no están "ocupadas" en el pasado
    assert len(disponibles) == 3
...
def test_disponibilidad_sin_mesas_en_sistema():
    """
    Caso Límite: Buscar disponibilidad en un restaurante que no tiene mesas registradas.
    """
    r_vacio = Restaurante()
